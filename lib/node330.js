...
var remoteServerViewer = require("./viewers/remoteServerViewer330.js");
...
var node330 = function (config) {
    ...
    this.logInfo("node330 created: program '"+path.basename(config.program)+
    ...
    this.injectVirtualComponentsIntoFunction = function (theFunction) {
        ...
        for (var argIndex = 0; argIndex < functionArgNames.length; argIndex++) {
            ...
            else if (_.str.startsWith(argNameLower, "frac") || _.str.endsWith(argNameLower, "frac")) {
                argValue = new components.virtualComponent330(valueTypes.FRACTION, argName);
            }
            else if (_.str.startsWith(argNameLower, "pid") || _.str.endsWith(argNameLower, "pid")) {
                argValue = new pid330.pid330();
            }
            else if (_.str.startsWith(argNameLower, "apparatus") || _.str.endsWith(argNameLower, "apparatus")) {
                // Get the name of the apparatus file
                var apparatusConfigFilename = self.config.apparatus;
                if (_.isUndefined(apparatusConfigFilename)) {
                    apparatusConfigFilename = (_.str.strRight(argNameLower, 'apparatus') +
                                               _.str.strLeft(argNameLower, 'apparatus'));
                }
                argValue = new config330(apparatusConfigFilename);
            }
            else if (_.str.startsWith(argNameLower, "config") || _.str.endsWith(argNameLower, "config")) {
                // Get the name of the config file
                var configFilename = self.config.config;
                if (_.isUndefined(configFilename)) {
                    configFilename = (_.str.strRight(argNameLower, 'config') +
                                      _.str.strLeft(argNameLower, 'config'));
                }
...
node330.prototype.createRemoteServerViewer = function (config) {
    return new remoteServerViewer(this, config);
}
...
node330.prototype.createPhysicalComponentWithSetterFunction = function (valueType, setterFunction) {
    var component = new components.physicalComponent330(valueType);

    component.setValue = function(value) {
        component.value = setterFunction(value);
    };
...
node330.prototype.run = function () {
    ...
    function startCurrentProgram() {
        ...
        // Create our www directory if it doesn't exist
        if (!fs.existsSync(process.cwd() + "/www")) {
            fs.mkdirSync(process.cwd() + "/www");
        }
        ...
        self.logInfo("Started a node330 UI web server on IP %s", self.webServer.address().address);
    ...
    if (_.isString(this.config.program)) {
        ...
        if (fs.existsSync(programFilename)) {
            ...
            throw new Error("Unable to find a program file named '" + programFilename + "'");
            return;
