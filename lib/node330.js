 ...
 * @class
 * @constructor
 * @param {function|string} setupFunction - A function that will be called one time when the program is first executed. Or a string to a file that will be loaded to get the functions for each
 ...
 */
var node330 = function(setupFunction, loopFunction, shutdownFunction)
{
    if(_.isString(setupFunction))
    {
        var theProgram = require(process.cwd() + "/" + setupFunction);
        setupFunction  = theProgram.setup;
        loopFunction = theProgram.loop;
        shutdownFunction = theProgram.shutdown;
    }
    ...
    if(!_.isUndefined(shutdownFunction))
    {
        if(!_.isFunction(shutdownFunction))
        {
            throw "shutdownFunction must be a function.";
        }

        this.shutdownFunction = shutdownFunction;
        this.injectVirtualComponentsIntoFunction(shutdownFunction);
    }
	...
	var app = express();
	app.listen(this.configuration.web_ui_server_port);
