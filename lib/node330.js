...
var webViewer = require("./viewers/webViewer330.js");
var consoleViewer = require("./viewers/consoleViewer330.js");
 ...
 * @class
 * @constructor
 * @param {string} programFile - A path to a program file to execute
 */
var node330 = function(programFile)
{
...
node330.prototype.createConsoleViewer = function(config)
{
	return new consoleViewer(this, config);
}

node330.prototype.createWebViewer = function(config)
{
	return new webViewer(this, config);
}
 ...
 * @param [isInitialState] {boolean} - Set to true if this will be the initial state of the engine.
 ...
 */
node330.prototype.defineState = function(stateName, loopFunction, isInitialState, enterFunction, leaveFunction)
{
	if(_.isUndefined(isInitialState))
	{
	...
	{
		throw "enterFunction must define a function."
	}

	if(!_.isUndefined(loopFunction) && !_.isFunction(loopFunction))
	{
		throw "loopFunction must define a function."
	}

	if(!_.isUndefined(leaveFunction) && !_.isFunction(leaveFunction))
	{
		throw "leaveFunction must define a function."
	}
	...
	if(isInitialState)
	{
		...
		{
			throw "An initial state has already been defined.";
		}
...
node330.prototype.defineStateEvent = function(eventName, startStates, endState)
{
	...
	this.stateMachine.events[eventName].startStates = startStates;
	this.stateMachine.events[eventName].endState = endState;
}
...
node330.prototype.raiseStateEvent = function(eventName)
{
	if(!_.isUndefined(this.stateMachine.events[eventName]))
	{
		...
		if(_.isUndefined(currentState) || _.contains(stateEvent.startStates, currentState.stateName))
		{
			var newStateName = this.stateMachine.events[eventName].endState;
			...
			{
				throw "The new state named '" + newStateName + "' does not exist."
			}
		...
		{
			throw "The event named '" + eventName + "' is not allowed from the current state of '" + currentState.stateName + "'";
		}
	}
}
...
node330.prototype.logWarning = function(message)
{
	var args = ["warn"];
...
node330.prototype.logError = function(message)
{
	var args = ["error"];
...
node330.prototype.run = function()
{
	...
	if(_.isString(this.programFile))
	{
		var programFilename = require.resolve(process.cwd() + "/" + this.programFile);
	...
	{
		throw "Must specify a program file";
	}
	...
	{
		if(!_.isFunction(this.setupFunction))
		{
			throw "setupFunction must be a function.";
		}

		this.injectVirtualComponentsIntoFunction(this.setupFunction);
	}

	if(!_.isUndefined(this.loopFunction))
	{
		if(!_.isFunction(this.loopFunction))
		{
			throw "loopFunction must be a function.";
		}

		this.injectVirtualComponentsIntoFunction(this.loopFunction);
	}

	if(!_.isUndefined(this.shutdownFunction))
	{
		if(!_.isFunction(this.shutdownFunction))
		{
			throw "shutdownFunction must be a function.";
		}

		this.injectVirtualComponentsIntoFunction(this.shutdownFunction);
	}

	// Setup our web based UI server
	var app = express();
	this.webUIApp = app;
	this.webServer = app.listen(this.configuration.web_ui_server_port);
	app.use(express.static(__dirname + '/../public'));

	this.logInfo("Started a node330 UI web server on port %s", this.configuration.web_ui_server_port);
	...
	this.logInfo("node330 has started running");
}
 ...
 * @param {function} shutdownFunction - A function that will be called when the program is stopped.
 * @returns {node330}
 */
node330.run = function(setupFunction, loopFunction, shutdownFunction)
{
	var node330Instance = new node330(setupFunction, loopFunction, shutdownFunction);
	node330Instance.run();
