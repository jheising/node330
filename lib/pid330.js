...
pid330.prototype.update = function(measuredValue, currentTime)
{
	var now;
    if (_.isUndefined(currentTime))
    {
        now = Date.now();
    }
    else
    {
        // Elapsed time in milliseconds since some arbitrary epoch.
        now = currentTime;
    }
	...
	var derivative = dt==0 ? 0 : (error - this.previousError) / dt;
	...
	if(!_.isUndefined(this.CVUpperLimit))
	{
		if(CV >= this.CVUpperLimit && error > 0)
		{
			integral = this.integral;
		}

		CV = Math.min(this.CVUpperLimit, CV);
	}

	if(!_.isUndefined(this.CVLowerLimit))
	{
		if(CV <= this.CVLowerLimit && error < 0)
		{
			integral = this.integral;
		}

		CV = Math.max(this.CVLowerLimit, CV);
	}
